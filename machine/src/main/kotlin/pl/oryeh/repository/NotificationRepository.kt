...
interface NotificationRepository : JpaRepository<Notification, Long> {
    ...
    @Query("INSERT INTO vending.notification(user_id, text, add_info) VALUES (:userId,:text,:link,:addInfo) RETURNING id", nativeQuery = true)
    fun create(userId: Int, text: String, link: String? = null, addInfo: String? = null): Long

    @Modifying @Query("UPDATE Notification SET read=:read WHERE id=:id")
    fun read(id: Long, read: Boolean = true)
    @Modifying @Query("UPDATE Notification SET read=:read WHERE id IN(:id)")
    fun read(id: Collection<Long>, read: Boolean = true)

    @Modifying @Query("UPDATE Notification SET read=:read WHERE user.id=:userId")
